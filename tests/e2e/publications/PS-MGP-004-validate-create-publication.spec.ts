...
test.describe('PS-MGP-004 - Creación exitosa de una publicación ', () => {
  const TEST_EMAIL = 'pruebasjos05@gmail.com';
  const TEST_PASSWORD = 'password123';
  const PUBLICATION_TITLE = 'Arroz Premium Extra Largo';
  const PUBLICATION_DESCRIPTION = 'Arroz de alta calidad, extra largo, ideal para acompañar tus comidas. Producto fresco y de primera calidad. Paquete de 1 kg.';
  const PUBLICATION_PRICE = '5.50';
  const PUBLICATION_CATEGORY = 'Alimentos y Bebidas';
  const IMAGE_PATH = path.join(process.cwd(), 'tests', 'e2e', 'publications', 'images', 'arroz.jpg');
  ...
  test('debería crear una nueva publicación exitosamente', async ({ page }) => {
    ...
    await test.step('Acceder a la sección Mis Productos', async () => {
      await page.waitForLoadState('networkidle');
    ...
    await test.step('Hacer clic en el botón "Nueva Publicación"', async () => {
      ...
      await page.waitForURL(/\/publicar|\/publish/, { timeout: 15000 });
      await page.waitForTimeout(1000);
    });
    ...
    await test.step('Llenar el campo Título', async () => {
      ...
      await titleField.fill(PUBLICATION_TITLE);
    ...
    await test.step('Seleccionar la categoría', async () => {
      await page.waitForTimeout(1000);
    ...
    await test.step('Enviar el formulario para crear la publicación', async () => {
    ...
    await test.step('Verificar que la publicación se creó exitosamente', async () => {
      await page.waitForLoadState('networkidle', { timeout: 15000 });
      await page.waitForTimeout(2000);
      const pageTitle = page.locator('text=/Mis Publicaciones/i').first();
      await expect(pageTitle).toBeVisible({ timeout: 10000 });
      const publicationCard = page.locator('text=/Arroz Premium|Arroz/i').first();
      await expect(publicationCard).toBeVisible({ timeout: 15000 });
      await page.waitForTimeout(1000);
    });
  });
});
