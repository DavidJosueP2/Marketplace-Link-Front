...
test.describe('PS-MGP-010 - Actualización fallida de una publicación', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/login');
    await page.waitForLoadState('networkidle');
  });

  test('debería mostrar errores cuando se intenta actualizar una publicación con datos faltantes', async ({ page }) => {
    ...
    await test.step('Verificar que el formulario de edición se carga con los datos existentes', async () => {
      await page.waitForLoadState('networkidle', { timeout: 15000 });
      await page.waitForTimeout(2000);
      const formTitle = page.locator('h1').filter({ hasText: /Editar|Actualizar/i }).first();
    ...
    await test.step('Borrar el campo Título dejándolo vacío', async () => {
      const titleField = page.locator('input#name').first();
    ...
    await test.step('Intentar actualizar la publicación sin el título', async () => {
      const updateButton = page.locator('button[type="submit"]').filter({ 
        hasText: /Actualizar|Guardar|Publicar/i 
      }).first();
    ...
    await test.step('Verificar que se muestra el error de validación para el campo título', async () => {
      const titleError = page.locator('text=/El título es obligatorio/i').first();
      await expect(titleError).toBeVisible({ timeout: 5000 });
      await page.waitForTimeout(1000);
    });

    await test.step('Presionar el botón "Volver" para regresar a Mis Productos', async () => {
      const backButton = page.locator('button').filter({ hasText: /Volver/i }).first();
