...
test.describe('PS-MGP-011 - Detección de contenido peligroso al actualizar una publicación', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/login');
    await page.waitForLoadState('networkidle');
  });

  test('debería detectar contenido peligroso al actualizar una publicación existente', async ({ page }) => {
    ...
    await test.step('Actualizar el campo Título con contenido peligroso (drogas)', async () => {
      const titleField = page.locator('input#name').first();
    ...
    await test.step('Intentar actualizar la publicación con contenido peligroso', async () => {
